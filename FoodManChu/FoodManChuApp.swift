...
struct FoodManChuApp: App {
    init() {
        UINavigationBar.appearance().titleTextAttributes = [
            NSAttributedString.Key.font: UIFontMetrics(forTextStyle: .body).scaledFont(for: UIFont(name: "Comfortaa-Bold", size: 18)!)
        ]
        
        UINavigationBar.appearance().largeTitleTextAttributes = [
            NSAttributedString.Key.font: UIFontMetrics(forTextStyle: .body).scaledFont(for: UIFont(name: "Comfortaa-Bold", size: 30)!)
        ]
        ...
        WindowGroup {
            RootView()
                ...
                .onAppear() {
                    ...
                    cloudKitManager.setupNetworkMonitor()
